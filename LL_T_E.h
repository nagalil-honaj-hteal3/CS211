...
class LL
{
 ...
 public:
  ...
  T deleteFront();//do not add an & sign because it will have warnings in the outputs
  T deleteRear();//do not add an & sign because it will have warnings in the outputs
  bool isEmpty();
  void displayAll();
...
void LL<T>::addFront(const T& el)
{
  if(isEmpty())//if there are no nodes inside of the heap memory
    {
      front = rear = new node<T>;//this can be either front or rear since both are pointing at the same node
      rear->elem = el;
    }
  else//if there are nodes inside the heap memory
    {
...
T LL<T>::deleteFront()
{
  if(isEmpty())//making sure that there are no values inside of this delete front function
    ...
    {
      T element = front->elem;//this will be used so the element will not be roaming around the heap memory
  ...
  else
    {
      node<T>* newFront;
      //newFront = new node<T>;
      newFront = front->next;//this must be important since we will be deleting the front so front has to shift to next
      T element = front->elem;//this element is important to store inside of the local variable
      delete front;
...
T LL<T>::deleteRear()
{
  if(isEmpty())//if there are no nodes inside of the heap memory
    ...
    {
      T old = rear->elem;//the element that has been stored
      delete rear;
	...
	;//do nothing. The purpose of this for loop is to advance p to second to last node
      rear = p;
...
******************************************************************************/
template <class T>
LL<T>::LL(const LL<T>& source)//the source will be the class name variable
{
...
void LL<T>::addInOrderAscend(const T& e)
{
  ...
  if(front == NULL && rear == NULL)//if there are no elements inside of the heap memory. IT IS IMPORTANT THAT THERE ARE TWO ASSIGNMENT OPERATORS AND NOT ONE!
    {
      front = rear = new node<T>;//front and rear will have the same node
	...
	{
	  node<T>* newFront;//to add a new front node
	...
	{
	  rear->next = new node<T>;//the new node after rear
	...
	{
	  node<T>* newNode;//the node that will be entered within the range
